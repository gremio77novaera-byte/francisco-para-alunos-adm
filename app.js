...
async function loadProfile(){
 ...
 const {data:p,error:pe}=await sb.from("profiles").select("id,full_name,role,role_id,enabled").eq("id",user.id).single();if(pe)throw pe;
 const {data:rps,error:re}=await sb.from("role_permissions").select("permissions(permission_key)").eq("role_id",p.role_id);
 if(re)throw re;
 p.permissions=(rps||[]).map(x=>x.permissions?.permission_key).filter(Boolean);
 return p;
}
...
async function startApp(){
 try{
  ...
  if(!profile||!profile.enabled)throw new Error("Usuário sem acesso.");
  $("#adminName").textContent=profile.full_name||"Administrador";
  $("#adminFullName").value=profile.full_name||"";
  await loadData();applyPermissions();
 }catch(e){$("#appView").classList.remove("active");$("#loginView").classList.add("active");$("#loginError").textContent=e.message||"Não foi possível carregar o painel."}
}
$("#loginForm").addEventListener("submit",async e=>{e.preventDefault();$("#loginError").textContent="";const {error}=await sb.auth.signInWithPassword({email:$("#email").value.trim(),password:$("#password").value});if(error){$("#loginError").textContent="E-mail ou senha incorretos.";return}await startApp()});
...
function openModal(html,onSave){$("#modalBody").innerHTML=html;$("#modal").classList.remove("hidden");$("#modalBody form").onsubmit=async e=>{e.preventDefault();try{await onSave(new FormData(e.target));$("#modal").classList.add("hidden")}catch(err){alert(err.message||"Erro ao salvar.")}}}
...
window.deleteGrade=async id=>{if(!confirm("Excluir este ano/série e suas turmas?"))return;const {error}=await sb.from("grades").delete().eq("id",id);if(error)return alert(error.message);await loadData();notify("Ano/série excluído.")};
...
window.deleteClass=async id=>{if(!confirm("Excluir esta turma?"))return;const {error}=await sb.from("school_classes").delete().eq("id",id);if(error)return alert(error.message);await loadData();notify("Turma excluída.")};
...
$("#noticeForm").addEventListener("submit",async e=>{e.preventDefault();const [kind,id]=$("#noticeTarget").value.split(":");const payload={title:$("#noticeTitle").value.trim(),body:$("#noticeText").value.trim(),image_url:$("#noticeImage").value.trim()||null,published:true,target_type:kind==="SCHOOL"?"SCHOOL":kind==="G"?"GRADE":"CLASS",target_id:kind==="SCHOOL"?null:id};const {error}=await sb.from("notices").insert(payload);if(error)return alert(error.message);e.target.reset();await loadData();notify("Aviso publicado.")});
window.deleteNotice=async id=>{if(!confirm("Excluir este aviso?"))return;const {error}=await sb.from("notices").delete().eq("id",id);if(error)return alert(error.message);await loadData();notify("Aviso excluído.")};
...
window.savePlaylist=async(e,id)=>{e.preventDefault();const fd=new FormData(e.target),existing=playlists.find(x=>x.grade_id===id),payload={grade_id:id,app_name:fd.get("app").trim(),playlist_url:fd.get("link").trim(),enabled:true,updated_at:new Date().toISOString()};const r=existing?await sb.from("playlists").update(payload).eq("id",existing.id):await sb.from("playlists").insert(payload);if(r.error)return alert(r.error.message);await loadData();notify("Playlist salva.")};
...
window.saveGames=async(e,id)=>{e.preventDefault();const fd=new FormData(e.target),existing=games.find(x=>x.grade_id===id),payload={grade_id:id,content:fd.get("games"),enabled:true,updated_at:new Date().toISOString()};const r=existing?await sb.from("weekly_games").update(payload).eq("id",existing.id):await sb.from("weekly_games").insert(payload);if(r.error)return alert(r.error.message);await loadData();notify("Jogos salvos.")};
...
function permissionCheckboxes(selected=[]){return permissions.map(p=>`<label class="permission-option"><input type="checkbox" name="perm" value="${esc(p.id)}" ${selected.includes(p.id)?"checked":""}><span><strong>${esc(p.name)}</strong><br><small>${esc(p.description||"")}</small></span></label>`).join("")}
$("#addRoleBtn").onclick=()=>openModal(`<h3>Criar cargo</h3><form class="form-grid"><label>Nome do cargo<input name="name" placeholder="Ex.: Agente Escolar" required></label><label>Descrição<textarea name="description" rows="2"></textarea></label><div><strong>Permissões</strong><div class="permission-grid">${permissionCheckboxes()}</div></div><button class="primary">Criar cargo</button></form>`,async fd=>{
 ...
 const ids=fd.getAll("perm");if(ids.length){const rows=ids.map(pid=>({role_id:r.id,permission_id:pid}));const {error:pe}=await sb.from("role_permissions").insert(rows);if(pe)throw pe}await loadData();notify("Cargo criado.");
});
window.editRole=async id=>{
 const r=roles.find(x=>x.id===id);const {data:rps,error}=await sb.from("role_permissions").select("permission_id").eq("role_id",id);if(error)return alert(error.message);const selected=(rps||[]).map(x=>x.permission_id);
 openModal(`<h3>Editar cargo</h3><form class="form-grid"><label>Nome<input name="name" value="${esc(r.name)}" required></label><label>Descrição<textarea name="description" rows="2">${esc(r.description||"")}</textarea></label><label class="check"><input name="enabled" type="checkbox" ${r.enabled?"checked":""}> Cargo ativo</label><div><strong>Permissões</strong><div class="permission-grid">${permissionCheckboxes(selected)}</div></div><button class="primary">Salvar</button></form>`,async fd=>{
  ...
  const {error:de}=await sb.from("role_permissions").delete().eq("role_id",id);if(de)throw de;const ids=fd.getAll("perm");if(ids.length){const {error:ie}=await sb.from("role_permissions").insert(ids.map(pid=>({role_id:id,permission_id:pid})));if(ie)throw ie}
  await loadData();notify("Cargo atualizado.");
 });
};
window.deleteRole=async id=>{const r=roles.find(x=>x.id===id);if(r.is_system)return alert("Este cargo do sistema é protegido.");if(!confirm("Excluir este cargo?"))return;const {error}=await sb.from("roles").delete().eq("id",id);if(error)return alert(error.message);await loadData();notify("Cargo excluído.")};
...
$("#adminForm").addEventListener("submit",async e=>{e.preventDefault();const name=$("#adminFullName").value.trim(),pass=$("#adminPassword").value,{data:{user}}=await sb.auth.getUser();const {error}=await sb.from("profiles").update({full_name:name}).eq("id",user.id);if(error)return alert(error.message);if(pass){const {error:pe}=await sb.auth.updateUser({password:pass});if(pe)return alert(pe.message)}profile.full_name=name;$("#adminName").textContent=name;$("#adminPassword").value="";notify("Configurações salvas.")});
